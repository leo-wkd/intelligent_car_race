...
void OLED_WrDat(u8 data)
{
  ...
  while(i--)
  {
    if(data&0x80){gpio_set(PTD9,1);}
    else{gpio_set(PTD9,0);}
    gpio_set(PTD8,1);
    asm("nop");
    gpio_set(PTD8,0);
    data<<=1;
...
{
  uint8 row=0,pixel=0,x=0;
  for(row=0;row<=6;row++)
  {
    OLED_Set_Pos(0,row);
    for(pixel=0;pixel<80;pixel++)
    {
      x=(0x01&img[row*8*80+pixel])\
	|((0x01&img[(row*8+1)*80+pixel])<<1)\
	  |((0x01&img[(row*8+2)*80+pixel])<<2)\
	    |((0x01&img[(row*8+3)*80+pixel])<<3)\
	      |((0x01&img[(row*8+4)*80+pixel])<<4)\
		|((0x01&img[(row*8+5)*80+pixel])<<5)\
	          |((0x01&img[(row*8+6)*80+pixel])<<6)\
	            |((0x01&img[(row*8+7)*80+pixel])<<7);
     OLED_WrDat(x);
    }
  }
  OLED_Set_Pos(0,7);
  for(pixel=0;pixel<80;pixel++)
    {
      x=(0x01&img[56*80+pixel])\
	|((0x01&img[57*80+pixel])<<1)\
	  |((0x01&img[58*80+pixel])<<2)\
	    |((0x01&img[59*80+pixel])<<3);
     OLED_WrDat(x);
    }
}
...
void OLED_PrintValueI(uint8 x, uint8 y, int16 data)
{
	if(data < 0)
	{
		OLED_6x8Char(x,y,'-');
	...
	{
	  OLED_6x8Char(x+6,y,ge+48);
	  OLED_6x8Char(x+12,y,32);
	  OLED_6x8Char(x+18,y,32);
	  OLED_6x8Char(x+24,y,32);
	}
	else if(qian==0 && bai==0)
	{
	  OLED_6x8Char(x+6,y,shi+48);
	  OLED_6x8Char(x+12,y,ge+48);
	  OLED_6x8Char(x+18,y,32);
	  OLED_6x8Char(x+24,y,32);
	}
	else if(qian==0)
	{
	  OLED_6x8Char(x+6,y,bai+48);
	  OLED_6x8Char(x+12,y,shi+48);
	  OLED_6x8Char(x+18,y,ge+48);
	  OLED_6x8Char(x+24,y,32);
	}
	else
	{
	  OLED_6x8Char(x+6,y,qian+48);
	  OLED_6x8Char(x+12,y,bai+48);
	  OLED_6x8Char(x+18,y,shi+48);
	  OLED_6x8Char(x+24,y,ge+48);
	}
}

//void OLED_PrintValueI(uchar x, uchar y, int data)
//{
//	uchar i,j,k,l,m;
//	if(data < 0)
//	{
...
//num小数位数最多四位
//==============================================================
 void OLED_PrintValueFP(uchar x, uchar y, uint16 data, uchar num)
 {
 	uchar m,i,j,k;
 	OLED_6x8Char(x, y, '.');
	m= data/1000;
 ...
 void OLED_PrintValueF(uchar x, uchar y, float data, uchar num)
 {
 	uchar l,m,i,j,k;  //万千百十个
 	uchar databiti = 6; //整数位数
 	uint16 tempdataui = 0;
  	int tempdataii = (int)data; //整数部分
 	long int tempdatalp = (long int)((data - (int)data)*10000); //取小数位后4位

 	//整数部分显示
 	if(data < 0.0000001)  OLED_6x8Char(x, y,'-');
 	else OLED_6x8Char(x, y,'+');
